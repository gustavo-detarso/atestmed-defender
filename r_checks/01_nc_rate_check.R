...
p_pct <- ifelse(p_row$n > 0, p_row$nc / p_row$n, 0)
o_pct <- ifelse(o_row$n > 0, o_row$nc / o_row$n, 0)

# IC 95% (Wilson) via prop.test
p_ci <- prop.test(p_row$nc, p_row$n)$conf.int
o_ci <- prop.test(o_row$nc, o_row$n)$conf.int
...
gg <- ggplot(plot_df, aes(Grupo, pct)) +
  ...
  labs(title="Taxa de Não Conformidade – Perito vs Demais (excl.)",
       subtitle=sprintf("Período: %s a %s  |  n=%d vs %d", opt$start, opt$end, p_row$n, o_row$n),
       y="Percentual", x=NULL) +
  theme_minimal(base_size=11) +
  theme(panel.grid.major.x = element_blank())

out <- file.path(export_dir, sprintf("rcheck_nc_rate_%s.png", perito_safe))
